...
@st.cache_data
def analyze_calories(ingredient_lines: list[str]) -> int:
    """
    ...
    r.raise_for_status()
    return r.json().get("calories", 0)

# ─── 4) Streamlit App ────────────────────────────────────────────────────────
st.set_page_config(page_title="Meal Plan", layout="wide")

# session-state must have been set earlier:
...
for label, col in zip(labels, cols):
    with col:
        ...
        for m in picks:
            ...
            # ingredient lines as list
            ingr_lines = [d[f"strMeasure{i}"].strip()+" "+d[f"strIngredient{i}"].strip()
                          for i in range(1,21)
